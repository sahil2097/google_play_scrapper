...
def fetch_review():
    ...
    if request.method == 'POST':
        try:
            ...
            Number_of_words = int(request.form['Number_of_words'])
            remove_stop_words = (request.form['remove_stop_words'])
            remove_stop_words = remove_stop_words.split(" ")
...
#                         logging.exception(e)
                    ...
                    stop_words = list(set((stop_words)+(remove_stop_words)))
                    ...
                    words_freq_desc1 = words_freq_desc[:Number_of_words]
                    ...
                    # plt.figure(4)
                    # plt.title('Bigram for Apps Description data')
...
#                         logging.exception(e)
                    ...
                    filtered_sentence = []
                    stop_words = (stopwords.words('english')) # stop words removal
                    stop_words = list(set((stop_words) + (remove_stop_words)))
                    ...
                    words_freq_desc1 = words_freq[:Number_of_words]
                    ...
                    # plt.figure(4)
                    # plt.title('Wordcloud for review data of apps')
                    # plt.imshow(wordCloud, interpolation='bilinear')
                    # plt.axis("off")
                    ...
                    # full_filename = os.path.join(appp.config['IMG_FOLDER'], img_file)
                    # full_filename = image_path
                    logging.info(' review word frequency created successfully')
                except Exception as e:
                    logging.info('Error occured in review analysis')
                    logging.exception(e)

            if (operation) == 'changes_analysis':  # Description analysis from description on google play store
                try:
                    logging.info('Recent changes analysis started')
                    result_details = []
                    for i in app_ids:
                        result_app = app(str(i)) # getting app details of app id's captured
                        del result_app['comments']
...
#                         logging.exception(e)
                    ...
                    for i in app_details['recentChangesHTML']:
                        df_chang_list.append(i)
                    changes_string = re.sub("[^A-Za-z" "]+", " ", str(df_chang_list)).lower()

                    changes_spelling_correction = []
                    spell = Speller(lang='en')
                    for i in changes_string.split():  # spell check
                        changes_spelling_correction.append(spell(i))
                    logging.info(" Spell check done")

                    lemmatizer = WordNetLemmatizer()
                    changes_lemmas = []
                    for i in changes_spelling_correction:  # converting to base words
                        changes_lemmas.append(lemmatizer.lemmatize(i))

                    stop_words = (stopwords.words('english'))  # stop words removal
                    new_words = ['br', 'bug', 'none', 'fix', 'fixed']
                    stop_words.extend(new_words)
                    stop_words = list(set((stop_words) + (remove_stop_words)))

                    changes_filtered_sentence = []
                    for w in changes_lemmas:  # stop words removal
                        if w not in stop_words:
                            changes_filtered_sentence.append(w)
                    change_bigrams_list = list(nltk.bigrams(changes_filtered_sentence))
                    change_dictionary2 = [' '.join(tup) for tup in change_bigrams_list]
                    from sklearn.feature_extraction.text import CountVectorizer
                    vectorizer = CountVectorizer(ngram_range=(2, 2))
                    change_bag_of_words = vectorizer.fit_transform(change_dictionary2)
                    # vectorizer.vocabulary_

                    sum_words_change = change_bag_of_words.sum(axis=0)
                    words_freq_change = [(word, sum_words_change[0, idx]) for word, idx in vectorizer.vocabulary_.items()]
                    words_freq_change = sorted(words_freq_change, key=lambda x: x[1], reverse=True)
                    words_freq_desc1 = words_freq_change[:Number_of_words]
                    # words_dict_change = dict(words_freq_change)
                    # WC_height = 1000
                    # WC_width = 1500
                    # WC_max_words = 100
                    # wordCloud = WordCloud(max_words=WC_max_words, height=WC_height, width=WC_width)

                    # wordCloud.generate_from_frequencies(words_dict_change)
                    # plt.figure(4)
                    # plt.title('Bigram Wordcloud for recent changes of the apps')
                    # plt.imshow(wordCloud, interpolation='bilinear')
                    # plt.axis("off")
                    # create path to save wc image
                    # image_path = os.path.join(appp.config['IMG_FOLDER'], searchstring +'_changes'+ '.png')
                    # plt.savefig(image_path)
                    #img_file = os.listdir(appp.config['IMG_FOLDER'])[0]

                    #full_filename = os.path.join(appp.config['IMG_FOLDER'], img_file)
                    # full_filename = image_path
                    logging.info(' word frequency created successfully')
                except Exception as e:
                    logging.info('Error in changes Analysis')
                    logging.exception(e)


            return render_template('wordfrequency.html', var= words_freq_desc1)
        except Exception as e:
            logging.exception(e)
    else:
        return render_template('index.html')
